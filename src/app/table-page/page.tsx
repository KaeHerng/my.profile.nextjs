...
export default function TablePage() {
  ...
  return (
    <div className="p-6 min-h-screen flex flex-col gap-12 bg-gradient-to-r from-blue-900 via-purple-900 to-pink-900 animate-gradient-x rounded-3xl">
      ...
      <div className="bg-white/20 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/30 p-4 flex flex-col gap-4">
        <h2 className="text-2xl font-bold text-white mb-2">Glass Table</h2>

        {/* Top Controls */}
        <div className="flex justify-between items-center flex-wrap gap-3">
          <div className="flex items-center gap-2">
            <label className="text-white font-medium">Rows per page:</label>
            <select
              value={pageSize}
              onChange={handlePageSizeChange}
              className="px-3 py-1 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-white/50 transition-all hover:scale-105 bg-white/10 text-white border-white/30"
            >
              {[5, 10, 20, 50].map((size) => (
                <option key={size} value={size}>
                  {size}
        ...
        <div className="overflow-auto max-h-[400px] rounded-2xl shadow-inner scrollbar-thin scrollbar-thumb-white/40 scrollbar-track-white/10 hover:scrollbar-thumb-white/60 transition-colors">
          <table className="min-w-[900px] w-full border-collapse text-sm text-white">
        ...
        <div className="flex justify-center items-center gap-2 mt-3 flex-wrap">
          <button
            onClick={handlePrev}
            disabled={page === 1}
            className="px-4 py-2 bg-white/20 text-white rounded-lg disabled:opacity-50 disabled:cursor-not-allowed transition-all hover:bg-white/30"
          >
            Prev
          </button>
          ...
          {Array.from({ length: totalPages }, (_, i) => i + 1)
            ...
            .map((p, idx, arr) => {
              ...
              return (
                <span key={p} className="flex items-center">
                  {showDots && <span className="px-2 text-white/50">...</span>}
                  <button
                    onClick={() => setPage(p)}
                    className={`px-3 py-1 rounded-md transition-all ${
                      p === page
